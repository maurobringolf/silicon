...
class TermToSMTLib2Converter(bookkeeper: Bookkeeper) extends PrettyPrinter with TermConverter[String, String, String] {
  ...
  protected def render(term: Term): Doc =  term match {
    ...
    case Quantification(quant, vars, body, triggers, name) =>
      ...
      val docTriggers =
        if (triggers.isEmpty || triggers.head.p.size == 0) empty
        else
      ...
      if (!(name.isEmpty && triggers.isEmpty))
        ...
        parens(docQuant <+> parens(docVars) <+> docBody)
    ...
    case bop: Iff => parens("not" <+> parens("xor" <> nest(group(line <> render(bop.p0) <> line <> render(bop.p1)))))
